    ...
    <td width=70%>
      ...
      <%
        for(SpecialInfo specialInfo:specialInfoList) {
          String selected = "";
          if(specialInfo.getSpecialNumber().equals(student.getSpecialObj().getSpecialNumber()))
            selected = "selected";
      %>
          <option value='<%=specialInfo.getSpecialNumber() %>' <%=selected %>><%=specialInfo.getSpecialName() %></option>
      <%
        }
      %>
    </td>
