...
public class SpecialInfoListHandler extends DefaultHandler {
	...
	public void characters(char[] ch, int start, int length) throws SAXException {
		...
		if (specialInfo != null) { 
            ...
            if ("collegeObj".equals(tempString)) 
            	specialInfo.setCollegeObj(valueString); 
            else if ("specialNumber".equals(tempString)) 
            	specialInfo.setSpecialNumber(valueString); 
            else if ("specialName".equals(tempString)) 
            	specialInfo.setSpecialName(valueString); 
            else if ("startDate".equals(tempString)) 
            	specialInfo.setStartDate(Timestamp.valueOf(valueString));
            else if ("introduction".equals(tempString)) 
            	specialInfo.setIntroduction(valueString); 
        } 
	}
