...
public class FriendEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_addTime.getText().toString().equals("")) {
						Toast.makeText(FriendEditActivity.this, "添加时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传好友信息信息*/
					FriendEditActivity.this.setTitle("正在更新好友信息信息，稍等...");
	...
	private void initViewData() {
		...
		this.TV_friendId.setText(friendId+"");
		for (int i = 0; i < studentList.size(); i++) {
			if (friend.getStudentObj1().equals(studentList.get(i).getStudentNumber())) {
				this.spinner_studentObj1.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < studentList.size(); i++) {
			if (friend.getStudentObj2().equals(studentList.get(i).getStudentNumber())) {
				this.spinner_studentObj2.setSelection(i);
				break;
			}
		}
		this.ET_addTime.setText(friend.getAddTime());
	}
	@Override
