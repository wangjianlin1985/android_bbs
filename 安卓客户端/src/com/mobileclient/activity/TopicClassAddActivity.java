...
public class TopicClassAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.topicclass_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加话题分类");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_topicClassName.getText().toString().equals("")) {
						Toast.makeText(TopicClassAddActivity.this, "分类名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传话题分类信息*/
					TopicClassAddActivity.this.setTitle("正在上传话题分类信息，稍等...");
					String result = topicClassService.AddTopicClass(topicClass);
